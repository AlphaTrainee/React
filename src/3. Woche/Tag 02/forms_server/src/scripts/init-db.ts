...
async function main() {
  ...
  if (shouldReset) {
    ...
    console.log("Option --reset aktiv: Alte Tabelle 'contact' wurde gelöscht.");
  }
  ...
  console.log("Tabelle 'contact' wurde geprüft/erstellt.");
  ...
  for (const form of samplecontact) {
    await db.execute({
      sql: "INSERT INTO contact (name, email, reason, notes, done) VALUES (?, ?, ?, ?, ?)",
...
main().catch(async (err) => {
  console.error("Fehler beim Seeding der contact-Tabelle:", err);
  try {
    await db.close();
  } catch (_) {}
  process.exit(1);
